...
% K5acT6ac/T6acK9ac
if His.rt_ref(8)-His.rt_ref(7)>0.4
    for hno=7:8
        [cur_rts,cur_intens,cur_mono_isointens] = get_histone10(MS1_index,MS1_peaks,ptol,unitdiff,His,hno);
        if cur_rts(1)>0
            pep_rts(hno,1:ncharge) = cur_rts;
...
t1 = His.rt_ref(1)-7;
t2 = His.rt_ref(1)+7;
[rts7,top1_rt7,inten_sum7] = get_rts(MS1_index,MS1_peaks,ptol,unitdiff,His,hno,nsplit,t1,t2);

[tmp_sum,ix] = sort(inten_sum7,'descend');
tmp_rts = rts7(ix);
if length(tmp_sum)>=2 && tmp_sum(2)>=tmp_sum(1)/30 && abs(tmp_rts(2)-tmp_rts(1))<2
    His.rt_ref(hno) = min([tmp_rts(2),tmp_rts(1)]);
    His.rt_ref(hno+1) = max([tmp_rts(2),tmp_rts(1)]);
...
t1 = His.rt_ref(1)-7;
t2 = His.rt_ref(1)+7;
[rts7,top1_rt7,inten_sum7] = get_rts2(MS1_index,MS1_peaks,MS2_index,MS2_peaks,ptol,unitdiff,His,hno,0,t1,t2,nhmass);
