function [cur_rts,cur_intens,cur_mono_isointens] = get_histone6(MS1_index,MS1_peaks,MS2_index,MS2_peaks,ptol,unitdiff,Mods,His,hno,special)
%%
...
[s_rts,ratio,cur_mono_isointens] = get_ratio_6iso(MS1_index,MS1_peaks,MS2_index,MS2_peaks,ptol,unitdiff,Mods,His,hno,h_rts,special);
...
function [s_rts,ratio,cur_mono_isointens] = get_ratio_6iso(MS1_index,MS1_peaks,MS2_index,MS2_peaks,ptol,unitdiff,Mods,His,hno,h_rts,special)%#ok
%%
...
if length(ms2pos)<=3
    ...
    s_rts(1:npart) = [His.rt_ref(hno) His.rt_ref(hno+1) His.rt_ref(hno+2) His.rt_ref(hno+3) His.rt_ref(hno+4) His.rt_ref(hno+5)];
    ...
    if 2~=special.nDAmode
        ...
        cur_mono_isointens(:,5) = [zeros([IX(1)-1,1]);n_inten1;zeros([num_MS1-IX(end),1])];
        cur_mono_isointens(:,6) = [zeros([IX(1)-1,1]);n_inten1;zeros([num_MS1-IX(end),1])];
    else
...
%     end;
    ...
    for pno = newpos
        ...
        r5 = 1.1*get_ratio(mz,inten,tol,K51,K52);
