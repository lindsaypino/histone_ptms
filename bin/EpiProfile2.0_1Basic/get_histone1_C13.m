function [cur_rts,cur_intens,cur_mono_isointens] = get_histone1_C13(MS1_index,MS1_peaks,ptol,unitdiff,His,hno)
%%
...
for jno=1:ncharge
    ...
    [c_isorts,c_ref_isointens] = GetProfiles(MS1_index,MS1_peaks,c_ref_isomzs,c_ch,ptol,1,rt_i1:rt_i2);%0->1
    j = 2;
    ...
    x = find(flag==1);
    if 0==isempty(x)
        %[tmp,id] = min(abs(c_isorts(nt(x))-His.rt_ref(hno)));
        ...
        cur_intens(jno) = get_area_C13(c_isorts,c_ref_isointens,nb,cur_pos,c_mz,c_ch,MS1_index,MS1_peaks,unitdiff,ptol);
    else
